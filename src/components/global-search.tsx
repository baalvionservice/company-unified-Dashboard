...
export default function GlobalSearch({ open, onOpenChange }: GlobalSearchProps) {
  ...
  const searchResults = {
      businesses: businesses.filter(b => b.name.toLowerCase().includes(search.toLowerCase())).slice(0,3),
      employees: employees.filter(e => e.name.toLowerCase().includes(search.toLowerCase())).slice(0,3),
      pages: navItems.filter(p => p.label.toLowerCase().includes(search.toLowerCase())).slice(0,3),
      reports: reports.filter(r => r.name.toLowerCase().includes(search.toLowerCase())).slice(0,3)
  }
      ...
      <CommandList>
        ...
        {searchResults.businesses.length > 0 && (
            ...
            {searchResults.businesses.map(biz => (
                <CommandItem key={biz.id} onSelect={() => handleSelect(`/businesses`)}>
        ...
        {searchResults.employees.length > 0 && (
            ...
            {searchResults.employees.map(emp => (
                <CommandItem key={emp.id} onSelect={() => handleSelect(`/employees/${emp.id}`)}>
        ...
        {searchResults.reports.length > 0 && (
            ...
            {searchResults.reports.map(report => (
                <CommandItem key={report.id} onSelect={() => handleSelect(`/finance/reports`)}>
