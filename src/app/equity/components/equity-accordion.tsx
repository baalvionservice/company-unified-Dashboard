      ...
      <CardContent>
        <Accordion type="single" collapsible defaultValue="item-biz_1">
          {allEquityData.map((data) => {
            const business = businessesData.find((b) => b.id === data.businessId);
              ...
              <AccordionItem value={`item-${business.id}`} key={business.id}>
                <AccordionTrigger>
                  <div className="flex items-center gap-3">
                    ...
                    <Badge variant="outline">Valuation: ${(data.valuation / 1_000_000).toFixed(0)}M</Badge>
                  </div>
                </AccordionTrigger>
                <AccordionContent>
                  <div className="grid grid-cols-1 xl:grid-cols-5 gap-6 p-2">
                    <div className="xl:col-span-3">
                        ...
                        <TableBody>
                            ...
                            <TableRow key={s.name}>
                                ...
                                <TableCell className="text-right font-mono">{s.equity.toFixed(2)}%</TableCell>
                                <TableCell className="text-right">${(s.usdValue / 1_000_000).toFixed(2)}M</TableCell>
                    ...
                    <div className="xl:col-span-2 flex items-center justify-center">
                        <EquityDistributionDonutChart stakeholders={data.stakeholders} />
                    </div>
                  </div>
                </AccordionContent>
              </AccordionItem>
