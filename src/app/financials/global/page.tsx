...
const globalStats = (() => {
  ...
  const totalCosts = serverCostsData.reduce((acc, cost) => acc + cost.cost, 0) * 12; // Annualized
...
const countryFinancials = countriesData.map(country => {
    const countryBusinesses = businesses.filter(b => b.country === country.name);
    const revenue = countryBusinesses.reduce((acc, b) => acc + b.currentMetrics.revenue / (fxRates[b.currency] || 1), 0);
    const profit = countryBusinesses.reduce((acc, b) => acc + b.currentMetrics.profit / (fxRates[b.currency] || 1), 0);
    const serverCost = (serverCostsData.find(sc => sc.country === country.name)?.cost || 0); // Monthly cost
...
export default function GlobalFinancialsPage() {
  ...
  const serverCostsWithRevenue = useMemo(() => {
    return serverCostsData.map(cost => {
      const countryBusinesses = businesses.filter(b => b.country === cost.country);
      const totalRevenue = countryBusinesses.reduce((acc, b) => acc + b.currentMetrics.revenue / (fxRates[b.currency] || 1), 0);
      const percentageOfRevenue = totalRevenue > 0 ? (cost.cost * 12 / totalRevenue) * 100 : 0;
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
            <h1 className="text-3xl font-bold tracking-tight">Global Financials</h1>
        ...
        <div className="space-y-8">
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
                 <Card>
                    ...
                    <CardContent>
                    <div className="text-2xl font-bold">${(globalStats.totalRevenue / 1000000).toFixed(1)}M</div>
                    </CardContent>
                </Card>
                 <Card>
                    <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                    <CardTitle className="text-sm font-medium">Total Costs (USD)</CardTitle>
                    <DollarSign className="h-4 w-4 text-muted-foreground" />
                    </CardHeader>
                    <CardContent>
                    <div className="text-2xl font-bold">${(globalStats.totalCosts / 1000000).toFixed(2)}M</div>
                    </CardContent>
                </Card>
                 <Card>
                    <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                    <CardTitle className="text-sm font-medium">Net Profit (USD)</CardTitle>
                    <DollarSign className="h-4 w-4 text-muted-foreground" />
                    </CardHeader>
                    <CardContent>
                    <div className="text-2xl font-bold">${(globalStats.netProfit / 1000000).toFixed(1)}M</div>
                    </CardContent>
                </Card>
                 <Card>
                    <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                    ...
                    <CardContent>
                    <div className="text-2xl font-bold">{globalStats.profitMargin.toFixed(1)}%</div>
                    </CardContent>
                </Card>
            </div>
            ...
            <Card>
                 ...
                 <CardContent className="pt-4">
                     <p className="text-sm text-muted-foreground">TechCorp India's launch of the new AI-powered analytics tool has driven a significant increase in monthly recurring revenue.</p>
                 </CardContent>
            </Card>
          ...
          {countryDetails && (
             <div>
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h2 className="text-2xl font-bold tracking-tight">{countryDetails.name} Financials</h2>
                    <p className="text-muted-foreground">A detailed look at your operations in {countryDetails.name}.</p>
                  </div>
                </div>
              </div>
          )}
          {countryBusinesses.length > 0 && <CountryPerformanceChart businesses={countryBusinesses} countryName={countryDetails?.name || ''} />}

          <Card>
             <CardHeader>
                 <CardTitle>Business Breakdown</CardTitle>
                 <CardDescription>All businesses operating within {countryDetails?.name}.</CardDescription>
             </CardHeader>
             <CardContent>
                 <Table>
                    <TableHeader>
                        ...
                        {countryBusinesses.map(biz => {
                            ...
                            return (
                                <TableRow key={biz.id}>
                                    <TableCell>
                                        <div className="flex items-center gap-3">
                                            <Avatar className="h-8 w-8">
                                                {image && <AvatarImage src={image.imageUrl} alt={biz.name} />}
                                    ...
                                    <TableCell className="text-right">{biz.currency} {(biz.currentMetrics.revenue / 1000000).toFixed(2)}M</TableCell>
                                     <TableCell className="text-right">{biz.currency} {(biz.currentMetrics.profit / 1000000).toFixed(2)}M</TableCell>
                                    <TableCell className="text-right">{biz.currentMetrics.employees}</TableCell>
                                </TableRow>
                            )
                        })}
                    </TableBody>
      ...
      {/* Server Costs Table */}
        <Card>
            <CardHeader>
                ...
                <CardDescription>Monthly infrastructure costs by country.</CardDescription>
            </CardHeader>
            <CardContent>
                <Table>
                    <TableHeader>
                    ...
                    <TableBody>
                        {serverCostsWithRevenue.map(cost => (
                             <TableRow key={cost.country}>
                                ...
                                <TableCell className="text-right">{cost.percentageOfRevenue.toFixed(1)}%</TableCell>
                            </TableRow>
                        ))}
                    </TableBody>
                </Table>
