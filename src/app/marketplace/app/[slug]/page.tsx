    ...
    <div className="space-y-8">
      <header className="flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          <Image src={app.icon} alt={`${app.name} logo`} width={80} height={80} className="rounded-lg border bg-card" data-ai-hint="logo" />
          <div>
        ...
        <div className="flex flex-col items-end gap-2">
            <div className="flex items-center gap-4">
                <div className="flex items-center gap-1">
                 ...
                 <div className="text-sm text-muted-foreground">{app.installs.toLocaleString()} installs</div>
            </div>
            {isInstalled ? (
                <Badge variant="secondary" className="text-base py-1 px-4"><CheckCircle className="mr-2 h-4 w-4" /> Installed</Badge>
            ) : (
                <Button size="lg" onClick={() => handleInstall(app)}>Install App</Button>
            )}
        </div>
      </header>
          ...
          <Tabs defaultValue="overview">
                   ...
                   {app.screenshots.length > 0 && (
                     <div className="grid grid-cols-2 gap-4">
                        {app.screenshots.map((ss, i) => (
                            <Image key={i} src={ss} alt={`Screenshot ${i+1}`} width={600} height={400} className="rounded-md border object-cover" data-ai-hint="app screenshot" />
                        ))}
                     </div>
                   )}
                </CardContent>
            ...
            <TabsContent value="reviews" className="mt-4">
                <div className="space-y-4">
                  ...
                  {app.reviews.length === 0 && <p className="text-muted-foreground">No reviews yet.</p>}
                </div>
            </TabsContent>
        ...
        <div className="lg:col-span-1 space-y-4 sticky top-24">
            <Card>
                ...
                <CardContent className="text-sm space-y-2">
                    <div className="flex justify-between"><span>Version</span> <span className="text-muted-foreground">{app.version}</span></div>
                    <div className="flex justify-between"><span>Last Updated</span> <span className="text-muted-foreground">{app.lastUpdated}</span></div>
                </CardContent>
            </Card>
            <Card>
                <CardHeader><CardTitle className="text-base">Permissions</CardTitle></CardHeader>
                <CardContent className="text-sm space-y-2">
                    {app.permissions.map(p => <div key={p} className="flex items-center gap-2"><CheckCircle className="h-4 w-4 text-green-500" />{p}</div>)}
                </CardContent>
            </Card>
        </div>
      </div>
