...
export default function PermissionsPage() {
  ...
  const handlePermissionChange = (role: Role, permission: string, value: boolean) => {
    setPermissions(prev => ({
      ...prev,
      [role]: {
        ...prev[role],
  ...
  const renderPermissionCell = (permissionValue: boolean | string) => {
    ...
    return <span className="text-xs text-muted-foreground">{permissionValue}</span>;
  };
                    ...
                    {roles.map(role => (
                      <TableCell key={role} className="text-center">
                        <div className="flex justify-center">
                          {renderPermissionCell(permissions[role][permission as keyof typeof permissions[Role]])}
                        </div>
                      </TableCell>
                    ))}
                  </TableRow>
      ...
      <Dialog open={isModalOpen} onOpenChange={setModalOpen}>
        <DialogContent className="max-w-4xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Toggle permissions for each role. Changes will apply immediately after saving.
            </DialogDescription>
          </DialogHeader>
            ...
            <Table>
                ...
                <TableBody>
                    ...
                    <TableRow key={permission}>
                        ...
                        <TableCell key={role} className="text-center">
                            <div className="flex justify-center">
                                {typeof permissions[role][permission as keyof typeof permissions[Role]] === 'boolean' ? (
                                    <Switch
                                        checked={permissions[role][permission as keyof typeof permissions[Role]] as boolean}
                                        onCheckedChange={(value) => handlePermissionChange(role, permission, value)}
                                    />
                                ) : (
                                    <AlertTriangle className="h-5 w-5 text-yellow-500" title="Complex permission"/>
                                )}
                            </div>
                        </TableCell>
                        ))}
