...
export default function ConsolidatedRevenueTable() {
  ...
  const totalGlobalRevenue = businessRevenues.reduce((acc, biz) => acc + biz.usdEquivalent, 0);

  return (
    <Card>
      <CardHeader>
        <CardTitle>Consolidated Revenue</CardTitle>
        <CardDescription>Business revenues shown in local currency and USD equivalent.</CardDescription>
      </CardHeader>
      <CardContent>
         <div className="rounded-md border">
          <Table>
            ...
            <TableBody>
              {businessRevenues.map(biz => (
                <TableRow key={biz.id}>
                  ...
                  <TableCell className="text-right">{biz.currentMetrics.revenue.toLocaleString()}</TableCell>
                  <TableCell>{biz.currency}</TableCell>
                  <TableCell className="text-right">${biz.usdEquivalent.toLocaleString(undefined, { maximumFractionDigits: 0 })}</TableCell>
                  <TableCell className="text-right font-mono">{biz.fxRate.toFixed(4)}</TableCell>
            ...
            <TableFooter>
                <TableRow>
                    <TableCell colSpan={3} className="font-bold">Total Global Revenue (USD)</TableCell>
                    <TableCell colSpan={3} className="text-right font-bold text-lg">${totalGlobalRevenue.toLocaleString(undefined, { maximumFractionDigits: 0 })}</TableCell>
                </TableRow>
            </TableFooter>
          </Table>
        </div>
