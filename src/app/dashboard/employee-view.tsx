...
const myTasks = [
    ...
    { id: 2, title: "Schedule posts for next week's campaign", status: "In Progress" },
    { id: 3, title: "Analyze competitor's new ad campaign", status: "In Progress" },
                 ...
                 <div className="grid auto-rows-max items-start gap-8 lg:col-span-2">
                    <div className="grid gap-4 md:grid-cols-3">
                        <Card>
                            ...
                            <CardContent>
                                <div className="flex items-center gap-2 text-2xl font-bold"><CheckCircle className="text-green-500"/>12</div>
                            </CardContent>
                        </Card>
                        <Card>
                            <CardHeader className="pb-2">
                            ...
                            <CardContent>
                                <div className="flex items-center gap-2 text-2xl font-bold"><Clock />38.5</div>
                            </CardContent>
                        </Card>
                        <Card>
                            <CardHeader className="pb-2">
                            ...
                            <CardContent>
                                <div className="flex items-center gap-2 text-2xl font-bold"><Star className="text-yellow-500" />4.8/5</div>
                            </CardContent>
                        </Card>
                    </div>
                     <Card>
                        <CardHeader>
                            ...
                            <CardDescription>Your current tasks for this week.</CardDescription>
                        </CardHeader>
                        <CardContent>
                            <ul className="space-y-4">
                                {myTasks.map(task => (
                                    <li key={task.id} className="flex items-center gap-4">
                                        {task.status === "Done" ? <CheckCircle className="h-5 w-5 text-green-500" /> : <Target className="h-5 w-5 text-muted-foreground"/>}
                                        <p className={`font-medium ${task.status === "Done" ? "text-muted-foreground line-through" : ""}`}>{task.title}</p>
                                        {task.status === "In Progress" && <Badge>In Progress</Badge>}
                                    </li>
                                ))}
                            </ul>
                        </CardContent>
