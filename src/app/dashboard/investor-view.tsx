...
export default function InvestorView() {
  ...
  const myBusinesses: Business[] = businesses.filter(b => investorBusinessIds.includes(b.id));
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">Investments across your portfolio</p>
          </CardContent>
        </Card>
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">Average equity holding</p>
          </CardContent>
        </Card>
      </div>
      ...
      <div>
        <h2 className="mb-4 text-2xl font-bold tracking-tight">My Businesses</h2>
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {myBusinesses.map(biz => {
            const image = PlaceHolderImages.find((img) => img.id === biz.imageId);
            const equity = biz.equitySplit.find(e => e.name === 'John Smith')?.percentage;
            return (
              <Card key={biz.id}>
                ...
                <CardContent className="grid gap-2">
                   <div className="text-sm text-muted-foreground">
                     Your Equity: <span className="font-bold text-foreground">
                        {equity || 25}%
                     </span>
                   </div>
                   <div className="text-sm text-muted-foreground">
                     Revenue: <span className="font-bold text-foreground">
                       {biz.currency} {(biz.currentMetrics.revenue / 1_000_000).toFixed(1)}M
                     </span>
                   </div>
                </CardContent>
              </Card>
