...
export default function ComparisonCharts() {
  const profitMarginData = [...comparisonData.profitMargins].sort((a,b) => a.margin - b.margin);
  const growthRatesData = [...comparisonData.growthRates].sort((a,b) => a.growth - b.growth);
          ...
          <ResponsiveContainer width="100%" height={300}>
            <BarChart data={comparisonData.revenueLast3Months}>
              ...
              {businessNames.map(name => (
                <Bar key={name} dataKey={name} fill={businessColors[name]} radius={[4, 4, 0, 0]} />
              ))}
            </BarChart>
          </ResponsiveContainer>
        </CardContent>
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={profitMarginData} layout="vertical">
                ...
                <YAxis dataKey="name" type="category" width={120} tick={{fontSize: 12}} />
                <Tooltip formatter={(value) => `${value}%`} />
                <Bar dataKey="margin" fill="hsl(var(--chart-4))" radius={[0, 4, 4, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
