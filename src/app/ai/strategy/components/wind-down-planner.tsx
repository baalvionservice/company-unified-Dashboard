...
import { Loader2, Zap, FileWarning, Users, Building, ShieldAlert } from 'lucide-react';
  ...
  return (
    <Card className="bg-muted/30">
      <CardHeader>
        ...
        <CardDescription>Model the financial and operational impact of closing a business unit.</CardDescription>
      </CardHeader>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="space-y-2">
            ...
            <Select value={business} onValueChange={setBusiness}>
              ...
              <SelectContent>{businessesData.map(b => <SelectItem key={b.id} value={b.id}>{b.name}</SelectItem>)}</SelectContent>
            </Select>
          </div>
          <div className="space-y-2 flex items-end">
            <Button onClick={handleAnalyze} disabled={loading} className="w-full" variant="destructive">
              {loading ? <Loader2 className="animate-spin" /> : 'Simulate Closure'}
            </Button>
          </div>
        </div>
