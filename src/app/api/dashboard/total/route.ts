...
export async function GET() {
  try {
    ...
    const total_revenue = (businesses as Business[]).reduce(
      (acc, biz) => acc + biz.currentMetrics.revenue / (rates[biz.currency] || 1),
      0
    );
    const total_profit = (businesses as Business[]).reduce(
      (acc, biz) => acc + biz.currentMetrics.profit / (rates[biz.currency] || 1),
      0
    );
    const total_expenses = total_revenue - total_profit;
    const profit_margin_percentage = total_revenue > 0 ? (total_profit / total_revenue) * 100 : 0;
