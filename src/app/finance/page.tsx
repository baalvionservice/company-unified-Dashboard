      ...
      <Card className="bg-muted/30">
        ...
        <CardContent className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
            <div className="md:col-span-1 space-y-4 rounded-lg bg-card p-6 shadow-sm">
                ...
                <p className="text-5xl font-bold tracking-tight">${(netWorth.total / 1_000_000).toFixed(1)}M</p>
                <div className="flex items-center text-green-500">
                 ...
                 <div className="space-y-2 pt-2 text-sm">
                    <div className="flex justify-between">
                        ...
                        <span className="font-medium">${(netWorth.breakdown.businessAssets / 1_000_000).toFixed(1)}M</span>
                    </div>
                     <div className="flex justify-between">
                        <span className="text-muted-foreground">Cash & Liquid</span>
                        <span className="font-medium">${(netWorth.breakdown.cashAndLiquid / 1_000_000).toFixed(1)}M</span>
                    </div>
                     <div className="flex justify-between">
                        <span className="text-muted-foreground">Investments</span>
                        <span className="font-medium">${(netWorth.breakdown.investments / 1_000_000).toFixed(1)}M</span>
                    </div>
                </div>
            </div>
            ...
            <Table>
                ...
                <TableBody>
                    {financialSummary.map(summary => (
                        <TableRow key={summary.businessId}>
                            <TableCell className="font-medium">{getBusinessName(summary.businessId)}</TableCell>
                            <TableCell className="text-right">${summary.revenue.toLocaleString()}</TableCell>
                            <TableCell className="text-right">${summary.totalCosts.toLocaleString()}</TableCell>
                            <TableCell className="text-right">${summary.grossProfit.toLocaleString()}</TableCell>
                            <TableCell className="text-right font-bold text-green-600">${summary.netProfit.toLocaleString()}</TableCell>
                            <TableCell className="text-right">
                                <Badge variant={summary.margin > 20 ? 'default' : 'secondary'}
                                 className={cn(summary.margin > 20 ? 'bg-green-100 text-green-800 border-green-300 dark:bg-green-950 dark:text-green-300' : 'bg-yellow-100 text-yellow-800 border-yellow-300 dark:bg-yellow-950 dark:text-yellow-300')}>
                ...
                <TableFooter>
                    <TableRow className="font-bold text-base">
                        ...
                        <TableCell className="text-right">${totals.totalCosts.toLocaleString()}</TableCell>
                        <TableCell className="text-right">${totals.grossProfit.toLocaleString()}</TableCell>
                        <TableCell className="text-right">${totals.netProfit.toLocaleString()}</TableCell>
