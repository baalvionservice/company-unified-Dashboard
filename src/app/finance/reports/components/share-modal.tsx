...
export default function CreatePortalModal({ isOpen, onOpenChange }: CreatePortalModalProps) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => { onOpenChange(open); setGeneratedUrl(''); setHasCopied(false); }}>
      <DialogContent className="sm:max-w-lg">
        ...
        {generatedUrl ? (
           <div className="space-y-4 py-4">
            ...
            <Alert>Your portal is now live. This link is active based on the expiry you set.</Alert>
           </div>
        ) : (
          <form onSubmit={handleSubmit} className="space-y-4 pt-4">
              <div className="space-y-2">
              <Label htmlFor="portal-name">Portal Name</Label>
              <Input id="portal-name" placeholder="e.g., Q4 Investor Update" required />
            </div>
              <div className="space-y-2">
              <Label htmlFor="investor-name">Investor Name</Label>
              <Input id="investor-name" placeholder="e.g., General Partners" required />
            </div>
            <div className="space-y-2">
              ...
              <div className="space-y-2 rounded-md border p-4 max-h-40 overflow-y-auto">
                {businessesData.map(biz => (
                    <div key={biz.id} className="flex items-center gap-2">
                      ...
                      <Label htmlFor={`modal_biz-${biz.id}`} className="font-normal">{biz.name}</Label>
                    </div>
                ))}
              </div>
            </div>
              ...
              <Label htmlFor="expiry">Expiry</Label>
                <Select defaultValue="30">
                  <SelectTrigger id="expiry"><SelectValue placeholder="Select expiry" /></SelectTrigger>
                  <SelectContent>
