...
export default function AttendanceTable() {
    ...
    const getBusinessName = (id: string) => businessesData.find(b => b.id === id)?.name || 'Unknown';

    return (
        <Card>
            <CardHeader>
                <CardTitle>Today's Attendance Log</CardTitle>
                <CardDescription>A detailed log of employee clock-in and clock-out times for today.</CardDescription>
            </CardHeader>
            <CardContent>
                        ...
                        <TableBody>
                            {todayLog.map(log => (
                                <TableRow key={log.employeeId}>
                                    <TableCell className="font-medium">{getEmployeeName(log.employeeId)}</TableCell>
