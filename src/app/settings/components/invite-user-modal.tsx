        ...
        <form onSubmit={handleSubmit} className="grid gap-4 py-4">
          ...
          <div className="space-y-2">
            ...
            <Select>
              ...
              <SelectContent>
                {roles.map(role => (
                  <SelectItem key={role} value={role}>{role.charAt(0) + role.slice(1).toLowerCase().replace('_', '-')}</SelectItem>
                ))}
              </SelectContent>
            </Select>
          </div>
           <div className="space-y-2">
            ...
            <div className="space-y-2 rounded-md border p-4 max-h-40 overflow-y-auto">
                <div className="flex items-center gap-2">
                    ...
                    <Label htmlFor="all-businesses" className="font-semibold">All Businesses</Label>
                </div>
                <Separator />
                {businessesData.map(biz => (
                    <div key={biz.id} className="flex items-center gap-2">
                      <Checkbox id={`biz-access-${biz.id}`} />
                      <Label htmlFor={`biz-access-${biz.id}`} className="font-normal">{biz.name}</Label>
                    </div>
                ))}
            </div>
          </div>
